...
import { eventButtonDesabled, eventCloseModal, eventSubmitDeleteModal, eventSubmitDeleteModalUser, eventSubmitEditModal, eventSubmitEditUser, eventSubmitModalCreate, eventSubmitToHire } from "./enventoBotao.js"
import { eventSelectInforUserEdit, eventSelectModal, eventSelectUserNoCompany } from "./eventoSelecionar.js"
...
export async function createModalBase(type, uuidCard='', uuidUser = '') {
    ...
    if (type == 'edit') {
        ...
        form.innerHTML = `
            ...
            <button type="submit" class="button-base button-roxoAzul-1" >Editar perfil</button>
        `

        modal.innerHTML = `
            <button class="button-close"><img src="../../assets/img/Vector (4).png" alt="icon close"></button>
            <h2 class="font-2-bold roxoAzul-2">Editar Perfil</h2>
        ` 
        ...
        btnCreate.classList.add("button-base", "button-roxoAzul-1")
        ...
        btnSave.classList.add("button-base", "button-roxoAzul-1")
        ...
        nameDep.classList.add("font-1-bold", "roxoAzul-2")

        let headerModal = document.createElement("header")
        headerModal.innerHTML = `
            <div class="container-company-infor">
                <h3 class="font-4-semibold cinza-2">${depFind.description}</h3>
                <span class="font-4-regular cinza-3">${depFind.companies.name}</span>
            </div>
        `
        ...
        currentDepartmentUsers.forEach((user) => {
            ...
            userTag.innerHTML = `
                <div>
                    <h2 class="font-4-semibold cinza-2">${user.username}</h2>
                    <span class="cinza-2">${user.professional_level !== null ? user.professional_level : "Ainda não definido"}</span>
                    <span class="cinza-3">${depFind.companies.name}</span>
                </div>
            `
