...
import { eventShowMenuCard } from "./mostrarMenu.js"
...
async function renderCompaniesHomePage(sector = '') {
    ...
    listApiCompanies.forEach((comp) => {
        ...
        li.innerHTML = `
            ...
            <div>
                <span class="font-4-regular cinza-3">${opening_hours} horas</span>
                <span class="font-4-semibold sector-btn roxoAzul-2">${sectors.description}</span>
            </div>
        `
...
async function renderInforUser() {
    ...
    containerInforUser.insertAdjacentHTML("beforeend", `
        <h2 class="font-3-semibold roxoAzul-2">${user.username}</h2>
        <ul class="list-infor">
            <li class="font-4-regular cinza-1">Email: ${user.email}</li>
        </ul>
    `)

    if (user.kind_of_work !== null) {
        let list = document.querySelector(".list-infor")
        list.insertAdjacentHTML("beforeend", `<li class="font-4-regular cinza-1">${user.kind_of_work}</li>`)
    } 

    if (user.professional_level !== null) {
        let list = document.querySelector(".list-infor")
        list.insertAdjacentHTML("beforeend", `<li class="font-4-regular cinza-1">${user.professional_level}</li>`)
    }
}
...
async function renderCompanyUser() {
    ...
    if (user.department_uuid !== null) {
        ...
        listCoWorkersApi.forEach((co) => {
            if (co.username !== user.username) {
                ...
                coWorkersTag.innerHTML = `
                    <h3 class="font-3-semibold roxoAzul-2">${co.username}</h3>
        ...
        containterJob.insertAdjacentHTML("afterbegin", '<h1 class="font-3-semibold roxoAzul-2">Você ainda não foi contratado</h1>')
    }
}
...
async function renderSelectCompany() {
    ...
    select.addEventListener("change", () => {
        ...
        if (companyTextCurrency == "Selecionar Empresa") {
            select.classList.remove("roxoAzul-2", "font-5-semibold")
            select.classList.add("cinza-4")
            renderDepartmentAll()
        } else {
            select.classList.remove("cinza-4")
            select.classList.add("roxoAzul-2", "font-5-semibold")
...
async function renderDepartmentAll(companySelected) {
    ...
    if (departments == 0) {
        ...
        textNotFoundCompanies.classList.add("roxoAzul-2")
        ...
        departments.forEach((depar) => {
            ...
            department.innerHTML = `
                <div class="infor-department">
                    <h3 class="cinza-2">${name}</h3>
                    <span class="cinza-2">${description}</span>
                    <span class="cinza-3">${companies.name}</span>
                </div>
                <nav class="nav-btns">
    ...
    listAllUserApi.forEach(async (user) => {
        if (user.username !== "ADMIN") {
            ...
            <div class="infor-user-registered">
                <h3 class="cinza-2">${username}</h3>
                <span class="cinza-2">${prof}</span>
                <span class="cinza-3">${depName}</span>
            </div>
            `
